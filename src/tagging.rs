...
pub fn tag_bool(output: &mut Vec<u8>, tag: &str, value: bool) {
    Tag::encode_from(tag, &TdfType::VarInt, output);
    value.encode(output);
}
...
pub fn tag_empty_str(output: &mut Vec<u8>, tag: &str) {
    Tag::encode_from(tag, &TdfType::String, output);
    encode_empty_str(output);
}

#[inline]
pub fn tag_empty_blob(output: &mut Vec<u8>, tag: &str) {
    Tag::encode_from(tag, &TdfType::Blob, output);
    output.push(0);
}
...
pub fn tag_group_start(output: &mut Vec<u8>, tag: &str) {
    Tag::encode_from(tag, &TdfType::Group, output);
}

#[inline]
pub fn tag_start(output: &mut Vec<u8>, tag: &str, ty: TdfType) {
...
#[inline]
pub fn tag_list_start(output: &mut Vec<u8>, tag: &str, ty: TdfType, len: usize) {
    Tag::encode_from(tag, &TdfType::List, output);
    ty.encode(output);
    len.encode(output);
}
...
pub fn tag_union_start(output: &mut Vec<u8>, tag: &str, key: u8) {
    Tag::encode_from(tag, &TdfType::Union, output);
    output.push(key);
}
...
#[inline]
pub fn tag_list_empty(output: &mut Vec<u8>, tag: &str, ty: TdfType) {
    Tag::encode_from(tag, &TdfType::List, output);
    ty.encode(output);
    output.push(0);
}
...
pub fn tag_var_int_list_empty(output: &mut Vec<u8>, tag: &str) {
    Tag::encode_from(tag, &TdfType::VarIntList, output);
    output.push(0);
}
...
pub fn tag_map_start(output: &mut Vec<u8>, tag: &str, key: TdfType, value: TdfType, len: usize) {
    Tag::encode_from(tag, &TdfType::Map, output);
    key.encode(output);
    value.encode(output);
    len.encode(output);
}
...
pub fn tag_triple<A: VarInt, B: VarInt, C: VarInt>(
    ...
    Tag::encode_from(tag, &TdfType::Triple, output);
    value.encode(output);
}

#[inline]
pub fn tag_pair<A: VarInt, B: VarInt>(output: &mut Vec<u8>, tag: &str, value: &(A, B)) {
    Tag::encode_from(tag, &TdfType::Pair, output);
    value.encode(output);
}

pub fn expect_list(reader: &mut Reader, tag: &str, value_type: TdfType) -> CodecResult<usize> {
    let _ = Tag::decode_until(reader, tag, TdfType::List)?;
    let list_type = TdfType::decode(reader)?;
...
pub fn expect_map(
    ...
    let _ = Tag::decode_until(reader, tag, TdfType::Map)?;
    let k_type = TdfType::decode(reader)?;
    let v_type = TdfType::decode(reader)?;
